...
(function($) {
	...
	RoyalSlider.prototype = {
			 ...
			 */			 
			goTo:function(id, silent, fromNav, fromTouch) {	
				...
				if(!this.isAnimating) {
					...
					if(fromTouch || silent || this.settings.slideTransitionType === "move") {
						...
						if(!this._animateCSS3Opacity) {											
							...
							setTimeout(function(){								
								newSlide.bind("webkitTransitionEnd", function(e) {								
									if(e.target == newSlide.get(0)) {	
										newSlide.unbind("webkitTransitionEnd");
										newSlide.remove();
										self._onSlideAnimationComplete();									
									}								
								});							
			...
			_moveSlideRight:function(fromTouch) {			
				if(this.currentSlideId < this.numSlides - 1) {
					this.goTo(this.currentSlideId+1, false, false, fromTouch);			
				} else {
					this.goTo(this.currentSlideId, false, false, fromTouch);			
				}		
			},
			_moveSlideLeft:function(fromTouch) {
				if(this.currentSlideId > 0) { 
					this.goTo(this.currentSlideId-1, false, false, fromTouch);			
				} else {
					this.goTo(this.currentSlideId, false, false, fromTouch);			
				...
				if(this.slideshowEnabled) {
					if(this.wasSlideshowPlaying ) {
						if(!this._isHovering) {
							...
							this._slideshowHoverLastState = true;
						}
						this.wasSlideshowPlaying = false;
						this._alreadyStoped = false;
					}
			...
			_scaleImage:function(img, containerWidth, containerHeight) {	
				...
				if(imgAlignCenter || imgScaleMode == "fill" || imgScaleMode == "fit") {			
					...
					function scaleImg () {						
						...
						if (imgAlignCenter) {
							//code to add frame around pics
							////alert("b: " + imgBorderSize);
							//alert("h: " + nHeight);
							vOffset = 323 - /*containerHeight/2 -*/ nHeight/2;
							if($(img.parents()[0]).hasClass("topbars")) {
								$(img.parents()[0]).css("width", nWidth);
								$(img.parents()[0]).css("height", nHeight);
								$(img.parents()[0]).css("margin-top", vOffset);
							}
							else {
								var newImg = img.wrap('<div class="sidebars" style="width:' + nWidth + 'px;height:' + nHeight + 'px;margin-top: ' + vOffset + 'px;" />');
								var wrapImg = newImg.wrap('<div class="topbars" style="height:' + nHeight + 'px;margin-top: ' + vOffset + 'px;" />');
								img = wrapImg;
							}
							//original code
